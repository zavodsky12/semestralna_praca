...
<div class="row">
    ...
    <div class="col-6 col-s-8">
        ...
        $userN = $_SESSION['name'];
        $sql = "SELECT id_nakupu as total FROM nakup JOIN pouzivatelia USING(id_pouzivatela) WHERE email LIKE '$userN'";
        $stmt = $conn->query($sql);
        $string = $stmt->fetch_assoc();
        if (!is_null($string)) {
        for ($i = $min; $i < $max+1; $i++) {
            $sql = "SELECT nakup.pocet_kusov as pocet_kusov, obrazok, nazov, cena FROM nakup JOIN produkty USING(id_produktu) JOIN pouzivatelia USING(id_pouzivatela) WHERE id_nakupu = '$i' AND email LIKE '$userN'";
            $stmt = $conn->query($sql);
            $string = $stmt->fetch_assoc();
            if (!is_null($string)) { ?>
            ...
            <div class="container">
                ...
                <button class="cervena"><b><a href="kosik.php">Odstrániť produkt</a></b></button>
                <br>
